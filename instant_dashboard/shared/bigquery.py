...
"""BigQuery tools for InstantDashboard.

This module contains BigQuery-related functionality migrated from data_science
...
def cleanup_sql(sql_string: str) -> str:
    ...
    # 3. Remove other common escape characters that might interfere
    sql_string = sql_string.replace("\\t", "\t")  # Convert escaped tabs to actual tabs
    sql_string = sql_string.replace("\\r", "\r")  # Convert escaped carriage returns
    
    # 4. Clean up any remaining backslashes that might be artifacts
    # Be careful not to remove legitimate backslashes in SQL patterns
...
def run_bigquery_validation(
    ...
    Returns:
        Dict[str, Any]: A dictionary with 'query_result' and 'error_message' keys.
    """
    
    # Clean up the SQL string
    sql_string = cleanup_sql(sql_string)
    
    # Check for dangerous DML/DDL operations
    dangerous_keywords = [
    ...
    for keyword in dangerous_keywords:
        if keyword in sql_upper:
            return {
                'query_result': None,
                'error_message': f"Dangerous SQL operation detected: {keyword}. Only SELECT queries are allowed."
            }
    
    try:
        client = get_bq_client()
        
        # Configure the query job with safety settings
        job_config = bigquery.QueryJobConfig(
            dry_run=False,  # We want actual results
        ...
        error_message = f"BigQuery validation error: {str(e)}"
