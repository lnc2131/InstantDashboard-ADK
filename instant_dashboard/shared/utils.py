...
"""Utility functions for InstantDashboard.

This module contains shared utility functions migrated from data_science
...
def extract_json_from_model_output(model_output: str) -> dict[str, Any]:
    """Extracts JSON object from a string that potentially contains markdown code fences.
    ...
    Returns:
        ...
        or a dict with 'error' key if JSON extraction fails.
    """
    try:
        cleaned_output = (
...
def get_image_bytes(filepath: str) -> bytes | None:
    ...
    Returns:
        The bytes of the image file, or None if the file does not exist or cannot be read.
    """
    try:
